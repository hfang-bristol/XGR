---
title: "XGR: eXploring Genomic Relations at the gene, SNP and genomic region level through enrichment, similarity, network and annotation analysis"
author: | 
...
R
library(rmarkdown)
rmarkdown::render("now.XGR_vignettes.Rmd")
#knitr::purl("now.XGR_vignettes.Rmd")
q('no')
################################################
```
...
We introduce an R package called *XGR*, publicly available at http://cran.r-project.org/package=XGR. This package is designed to make a user-defined gene or SNP list more interpretable by comprehensively utilising ontology and network information to reveal relationships and enhance opportunities for biological discovery. XGR is unique in supporting a broad range of ontologies (including knowledge of biological and molecular functions, pathways, diseases and phenotypes - in both human and mouse) and different types of networks (including functional, physical and pathway interactions).

In this user manual, you will be guided through the steps necessary to use this tool. After going through particularly the `Showcases` section which includes several demos with published data, you will be able to: 1) perform enrichment analysis using either built-in or custom ontologies, 2) calculate semantic similarity between genes (or between SNPs) based on their ontology annotation profiles, 3) identify a gene subnetwork given your query list of (significant) genes or SNPs, and 4) interpret genomic regions using co-localised functional genomic annotations and using nearby gene annotations by ontologies. For end-users who are unfamiliar with R, please refer to our user-friendly web app [^1].

[^1]: <http://galahad.well.ox.ac.uk/XGR>


# Web-app ([linkto](http://galahad.well.ox.ac.uk/XGR))

# Installation

We assume [R](http://cran.r-project.org), a language and environment for statistical computing and graphics, has been installed. For installation of the `XGR` package, please follow the instructions below:

* Install the the `XGR` package and its dependencies:
```{r eval=F}
source("http://bioconductor.org/biocLite.R")
biocLite("XGR")
```

* Highly recommend to install the development version of `XGR` package (now hosted in *GitHub* [^2]):
```{r eval=F}
...
### xEnrichConciser
*[`xEnrichConciser`](http://rawgit.com/hfang-bristol/XGR/master/inst/xEnrichConciser.html)*: makes enrichment results much clearer by removing redundant terms. A redundant term is claimed if its overlapped part with a more significant term meets both criteria: covers more than 95% of this redundant term but no more than 50% of the more significant term. In doing so, only non-redundant but informative terms will be left. 

### xEnrichBarplot
*[`xEnrichBarplot`](http://rawgit.com/hfang-bristol/XGR/master/inst/xEnrichBarplot.html)*: visualises enrichment results using a barplot.

### xEnrichDAGplot
*[`xEnrichDAGplot`](http://rawgit.com/hfang-bristol/XGR/master/inst/xEnrichDAGplot.html)*: visualises enrichment results using a DAG plot. This function is only useful for tree-like structured ontologies. Significant terms (of interest) are highlighted by box-shaped nodes, and the others by ellipse nodes. 

### xEnrichCompare
*[`xEnrichCompare`](http://rawgit.com/hfang-bristol/XGR/master/inst/xEnrichCompare.html)*: compares enrichment results using side-by-side barplots. This function is useful when comparing enrichment results for different inputs but based on the same ontology. 

### xEnrichDAGplotAdv
*[`xEnrichDAGplotAdv`](http://rawgit.com/hfang-bristol/XGR/master/inst/xEnrichDAGplotAdv.html)*: visualises comparative enrichment results using a DAG plot. This function takes input the output of the function `xEnrichCompare` to further illustrate differences and commonalities of comparative enrichment results in the context of ontology tree.
...
### xCircos
*[`xCircos`](http://rawgit.com/hfang-bristol/XGR/master/inst/xCircos.html)*: visualises the similarity results using a circos plot. The degree of similarity between SNPs (or genes) is visualised by the colour of links. This function can be used either to visualise the most similar links or to plot links involving an input SNP (or gene).  

### xSocialiserDAGplot
*[`xSocialiserDAGplot`](http://rawgit.com/hfang-bristol/XGR/master/inst/xSocialiserDAGplot.html)*: visualises terms used to annotate an input SNP (or gene) using a DAG plot. Terms used for direct/original annotations by box-shaped nodes, and terms for indirect/inherited annotations by ellipse nodes. This function is part of utilities in understanding calculated similarity.  

### xSocialiserDAGplotAdv
*[`xSocialiserDAGplotAdv`](http://rawgit.com/hfang-bristol/XGR/master/inst/xSocialiserDAGplotAdv.html)*: uses a DAG plot to visualise and compare two sets of terms used to annotate two input SNPs (or genes) that are predicted to be similar. This function is part of utilities in understanding calculated similarity.
...
### xSNP2GeneScores
*[`xSNP2GeneScores`](http://rawgit.com/hfang-bristol/XGR/master/inst/xSNP2GeneScores.html)*: takes as input a list of user-defined SNPs with the significance level (eg GWAS reported p-values), and defines and scores nearby genes that take into account the distance to and the significance of input SNPs.
...
Annotation functions are supposed to interpret user-defined list of genomic regions either via looking at nearby gene annotations by ontologies or via looking at co-localised functional genomic annotations. 

### xGRviaGeneAnno
*[`xGRviaGeneAnno`](http://rawgit.com/hfang-bristol/XGR/master/inst/xGRviaGeneAnno.html)*: conducts region-based enrichment analysis using nearby gene annotations, including two major steps. The first step is to define nearby genes within the maximum distance gap between genomic regions and gene location. The second step is to use `xEnricherGenes` for enrichment analysis to identify enriched terms. 

### xGRviaGenomicAnno
*[`xGRviaGeneAnno`](http://rawgit.com/hfang-bristol/XGR/master/inst/xGRviaGenomicAnno.html)*: conducts region-based enrichment analysis using functional genomic annotations. Enrichment analysis is based on binomial test for estimating the significance of overlaps at the base resolution. Genomic annotations cover a broad spectrum of genetic and epigenetic knowledge, including functional genomic data experimentally generated by the consortia (such as ENCODE, FANTOM, Blueprint and TCGA) and comparative genomic data predicted by the computational methods.

### xGRviaGenomicAnnoAdv
*[`xGRviaGenomicAnnoAdv`](http://rawgit.com/hfang-bristol/XGR/master/inst/xGRviaGenomicAnnoAdv.html)*: conducts region-based enrichment analysis using functional genomic annotations. Enrichment analysis is achieved by comparing the observed overlaps against the expected overlaps which are estimated from the null distribution. The null distribution is generated via sampling, that is, randomly generating samples for data genomic regions from background genomic regions. Background genomic regions can be provided by the user; by default, the annotatable genomic regions will be used. Since sampling is time-consuming, the parallel computation is also supported for Unix-like computers. 
...
* *[`xDAGsim`](http://rawgit.com/hfang-bristol/XGR/master/inst/xDAGsim.html)*: calculates semantic similarity between terms, and returns a network with nodes for terms and edges for pair-wise semantic similarity between terms.
...
* *[`xVisKernels`](http://rawgit.com/hfang-bristol/XGR/master/inst/xVisKernels.html)*: visualises distance kernel functions. 
* *[`xSNPscores`](http://rawgit.com/hfang-bristol/XGR/master/inst/xSNPscores.html)*: calculates scores for lead or LD SNPs. 
* *[`xSNP2nGenes`](http://rawgit.com/hfang-bristol/XGR/master/inst/xSNP2nGenes.html)*: defines nearby genes from SNPs.
* *[`xSparseMatrix`](http://rawgit.com/hfang-bristol/XGR/master/inst/xSparseMatrix.html)*: creates a sparse matrix for an input file.
* *[`xLiftOver`](http://rawgit.com/hfang-bristol/XGR/master/inst/xLiftOver.html)*: lifts genomic intervals from one genome build to another.
* *[`xGRsampling`](http://rawgit.com/hfang-bristol/XGR/master/inst/xGRsampling.html)*: generates random samples for data genomic regions from background genomic regions.
...
The identified gene network with nodes colored according to FDR is shown below. Notably, if nodes appear abnormally, please remove `vertex.shape="sphere"` when running the function `xVisNet`.
...
subnet_SNP <- xSubneterSNPs(data=data, network="STRING_high", distance.max=200000, seed.genes=T, subnet.significance=1e-1, subnet.size=50)
```

The identified gene network with nodes colored according to scores is shown below. Notably, if nodes appear abnormally, please remove `vertex.shape="sphere"` when running the function `xVisNet`.
...
In the following subsections, we focus on 3 diseases, Crohn's Disease (CRO), Celiac Disease (CEL) and Inflammatory Bowel Disease (IBD), identifying putative gene networks that are likely modulated by their corresponding lead (or LD) SNPs. The applications in the other diseases can be similarly done.
...
subnet_CRO <- xSubneterSNPs(data=data, network="STRING_high", distance.max=200000, seed.genes=T, subnet.size=25)
```

The identified gene network with nodes colored according to scores (the higher the more signficant) is shown below. Notably, if nodes appear abnormally, please remove `vertex.shape="sphere"` when running the function `xVisNet`.
...
# find maximum-scoring gene subnetwork with the desired node number=30
subnet_CEL <- xSubneterSNPs(data=data, network="STRING_high", distance.max=200000, seed.genes=T, subnet.size=25)
```
...
### SNP-modulated gene network in Inflammatory Bowel Disease

```{r, eval=T, cache=T}
# get SNPs reported in IBD GWAS and their significance info (p-values)
gr <- ImmunoBase$IBD$variant
data <- as.matrix(mcols(gr)[, c('Variant','Pvalue')])

# find maximum-scoring gene subnetwork with the desired node number=30
subnet_IBD <- xSubneterSNPs(data=data, network="STRING_high", distance.max=200000, seed.genes=T, subnet.size=25)
```

The identified gene network with nodes colored according to scores (the higher the more signficant) is shown below. Notably, if nodes appear abnormally, please remove `vertex.shape="sphere"` when running the function `xVisNet`.

```{r, fig.width=8, fig.height=8, eval=T, cache=T, echo=T}
pattern <- -log10(as.numeric(V(subnet_IBD)$significance))
pattern[is.infinite(pattern)] <- max(pattern[!is.infinite(pattern)])
vmax <- ceiling(stats::quantile(pattern, 0.75))
vmin <- floor(min(pattern))
xVisNet(g=subnet_IBD, pattern=pattern, glayout=layout_(subnet_IBD, with_kk()), vertex.shape="sphere", colormap="yr", zlim=c(vmin,vmax), newpage=F, edge.arrow.size=0.3, vertex.label.color="blue", vertex.label.dist=0.35, vertex.label.font=2)
```

> Identify pathways enriched with genes in the identified network

```{r, eval=T, cache=T, results="hide"}
data <- V(subnet_IBD)$name
eTerm <- xEnricherGenes(data=data, ontology="MsigdbC2CPall")
# view enrichment results for the top significant terms
xEnrichViewer(eTerm)
# save enrichment results to the file 'IBD_Pathway_enrichments.txt'
res_IBD_PW <- xEnrichViewer(eTerm, top_num=length(eTerm$adjp), sortBy="adjp", details=F)
output <- data.frame(term=rownames(res_IBD_PW), res_IBD_PW)
write.table(output, file="IBD_Pathway_enrichments.txt", sep="\t", row.names=F)
```

Enrichment results for the top 10 significant pathways are shown below:

```{r, eval=T, echo=F}
knitr::kable(res_IBD_PW[1:10,], digits=20, caption="", row.names=F)
```
...
data <- as.matrix(mcols(gr)[, c('Variant','Pvalue')])
mSeed <- xSNP2GeneScores(data, include.LD=NA, LD.r2=0.8, significance.threshold=5e-5, distance.max=200000, decay.kernel=c("slow","linear","rapid"), decay.exponent=2, GR.SNP="dbSNP_GWAS", GR.Gene="UCSC_genes", scoring.scheme=c("max","sum","sequential"), verbose=T, RData.location=RData.location)
...
ind <- match(c('SLE','ATD','CEL','T1D','RA','PSO','AS','IBD','CRO','UC'), names(ImmunoBase))
IB_selected <- ImmunoBase[ind]

res_name <- lapply(IB_selected, function(x) x$disease)
res_fullname <- unlist(res_name)

res_seed <- lapply(IB_selected, function(x){
	if(T){
		now <- Sys.time()
		message(sprintf("Analysing %s (%s) ...", x$disease, as.character(now)), appendLF=T)
	}
	gr <- x$variant
	data <- GenomicRanges::mcols(gr)[, c('Variant','Pvalue')]
	xSNP2GeneScores(data, include.LD=NA, LD.r2=0.8, significance.threshold=5e-8, distance.max=50000, decay.kernel="rapid", decay.exponent=2, GR.SNP="dbSNP_Common", GR.Gene="UCSC_genes", scoring.scheme="max", verbose=T, RData.location=RData.location)
})

## all SNPs
res_SNP <- lapply(res_seed, function(x) x$SNP$SNP)
allSNPs <- unique(unlist(res_SNP))
## all Genes
res_Gene <- lapply(res_seed, function(x) x$Gene$Gene)
allGenes <- unique(unlist(res_Gene))

## for SNP x Disease matrix
nm <- res_fullname
mat_SNP <- 0
for(i in 1:length(res_seed)){
	x <- res_seed[[i]]
	df <- x$SNP
	df$Disease <- rep(nm[i], nrow(df))
	mat <- xSparseMatrix(df[,c("SNP","Disease","Score")], rows=allSNPs, columns=nm, verbose=verbose)
	mat_SNP <- mat_SNP + mat
}
mat_SNP <- as.matrix(mat_SNP)

## for Gene x Disease matrix
nm <- res_fullname
mat_Gene <- 0
for(i in 1:length(res_seed)){
	x <- res_seed[[i]]
	df <- x$Gene
	df$Disease <- rep(nm[i], nrow(df))
	mat <- xSparseMatrix(df[,c("Gene","Disease","Score")], rows=allGenes, columns=nm, verbose=verbose)
...
colnames(data) <- gsub('\\)', '', colnames(data))
visTreeBootstrap(data=t(data), algorithm="nj", metric=c("euclidean","pearson","spearman","cos","manhattan","kendall","mi")[1], num.bootstrap=10000, reroot="min.bootstrap", visTree=T, nodelabels.arg=list(cex=0.8))
### for Gene
data <- mat_Gene
colnames(data) <- gsub('.*\\(', '', colnames(data))
colnames(data) <- gsub('\\)', '', colnames(data))
tree_bs <- visTreeBootstrap(data=t(data), algorithm="nj", metric="euclidean", num.bootstrap=10000, reroot="min.bootstrap", visTree=T, nodelabels.arg=list(cex=0.8))
tree_bs_ind <- match(tree_bs$tip.label, colnames(data))

## heatmap
### for Gene
data <- mat_Gene[, rev(tree_bs_ind)]
visHeatmapAdv(data, Rowv=T, Colv=F, dendrogram="none", dist.metric="euclidean", colormap="lightblue-blue", labRow=NA, KeyValueName="Gene score", zlim=c(0,6), add.expr=abline(v=(1:(ncol(data)+1))-0.5,col="white"), lmat=rbind(c(4,3), c(2,1)), lhei=c(1,4), lwid=c(1,3), margins=c(12,2), srtCol=30, cexCol=0.8)
### for SNP
data <- mat_SNP[, rev(tree_bs_ind)]
visHeatmapAdv(data, Rowv=T, Colv=F, dendrogram="none", dist.metric="euclidean", linkage.method="average", colormap="lightblue-blue", labRow=NA, KeyValueName="SNP score", zlim=c(0,6), add.expr=abline(v=(1:(ncol(data)+1))-0.5,col="white"), lmat=rbind(c(4,3), c(2,1)), lhei=c(1,4), lwid=c(1,3), margins=c(10,2), srtCol=30, cexCol=0.8)


## consensus tree
### for SNP
tree_SNPs <- visTreeBootstrap(data=t(mat_SNP), algorithm="nj", metric=c("euclidean","pearson","spearman","cos","manhattan","kendall","mi")[1], num.bootstrap=5000, consensus=T, consensus.majority=0.5, reroot="min.bootstrap", visTree=T, nodelabels.arg=list(cex=0.8))
tree_cons <- tree_SNPs
### for Gene
tree_Genes <- visTreeBootstrap(data=t(mat_Gene), algorithm="nj", metric=c("euclidean","pearson","spearman","cos","manhattan","kendall","mi")[1], num.bootstrap=5000, consensus=T, consensus.majority=0.8, reroot="min.bootstrap", visTree=T, nodelabels.arg=list(cex=0.5))
tree_cons <- tree_Genes
### visualisation
library(ape)
library(igraph)
g <- as.igraph(tree_cons)
vertex.label <- V(g)$name
vertex.label <- gsub("Node.*", "", vertex.label)
vertex.color <- rep("red", vcount(g))
vertex.color[vertex.label==''] <- 'yellow'
xVisNet(g=g, glayout=layout_with_kk, vertex.shape="sphere", vertex.label.dist=0.2, vertex.label.color="black", vertex.label=vertex.label, vertex.color=vertex.color)
...
mSeed <- xSNP2GeneScores(data, include.LD=NA, LD.r2=0.8, significance.threshold=5e-8, distance.max=50000, decay.kernel="rapid", decay.exponent=2, GR.SNP="dbSNP_Common", GR.Gene="UCSC_genes", scoring.scheme=c("max","sum","sequential"), verbose=T, RData.location=RData.location)

data <- GenomicRanges::mcols(inGR)[, c('Variant','Pvalue')]
# find maximum-scoring gene subnetwork with the desired node number=30
subnet_in <- xSubneterSNPs(data=data, include.LD=NA, LD.r2=0.8, significance.threshold=5e-8, distance.max=50000, network="STRING_high", decay.kernel="rapid", decay.exponent=2, GR.SNP="dbSNP_Common", GR.Gene="UCSC_genes", scoring.scheme="max", verbose=T, RData.location=RData.location, seed.genes=T, subnet.size=20)

# visualise network
subnet <- subnet_in
pattern <- -log10(as.numeric(V(subnet)$significance))
pattern[is.infinite(pattern)] <- max(pattern[!is.infinite(pattern)])
xVisNet(g=subnet, pattern=pattern, vertex.shape="sphere", colormap="white-green-darkgreen", zlim=c(0,6), newpage=F, vertex.size=12, vertex.label.color="blue", vertex.label.dist=0.5, vertex.label.font=3, glayout=layout_with_kk)


######################################
## polygenic autoimmune diseases
######################################
autoimmu <- c('SLE','ATD','CEL','T1D','RA')
...
mSeed <- xSNP2GeneScores(data, include.LD=NA, LD.r2=0.8, significance.threshold=5e-8, distance.max=50000, decay.kernel="rapid", decay.exponent=2, GR.SNP="dbSNP_Common", GR.Gene="UCSC_genes", scoring.scheme="max", verbose=T, RData.location=RData.location)

data <- GenomicRanges::mcols(imGR)[, c('Variant','Pvalue')]
# find maximum-scoring gene subnetwork with the desired node number=30
subnet_im <- xSubneterSNPs(data=data, include.LD=NA, LD.r2=0.8, significance.threshold=5e-8, distance.max=50000, network="STRING_high", decay.kernel="rapid", decay.exponent=2, GR.SNP="dbSNP_Common", GR.Gene="UCSC_genes", scoring.scheme="max", verbose=T, RData.location=RData.location, seed.genes=T, subnet.size=20)

subnet <- subnet_im
pattern <- -log10(as.numeric(V(subnet)$significance))
pattern[is.infinite(pattern)] <- max(pattern[!is.infinite(pattern)])
xVisNet(g=subnet, pattern=pattern, vertex.shape="sphere", colormap="white-red-darkred", zlim=c(0,6), newpage=F, vertex.size=12, vertex.label.color="blue", vertex.label.dist=0.5, vertex.label.font=3, glayout=layout_with_kk)


######################################
## mixture pattern disease
######################################
mix <- c('AS','PSO')
...
data <- GenomicRanges::mcols(mxGR)[, c('Variant','Pvalue')]
# find maximum-scoring gene subnetwork with the desired node number=30
subnet_mx <- xSubneterSNPs(data=data, include.LD=NA, LD.r2=0.8, significance.threshold=5e-8, distance.max=50000, network="STRING_high", decay.kernel="rapid", decay.exponent=2, GR.SNP="dbSNP_Common", GR.Gene="UCSC_genes", scoring.scheme="max", verbose=T, RData.location=RData.location, seed.genes=T, subnet.size=20)

subnet <- subnet_mx
pattern <- -log10(as.numeric(V(subnet)$significance))
pattern[is.infinite(pattern)] <- max(pattern[!is.infinite(pattern)])
xVisNet(g=subnet, pattern=pattern, vertex.shape="sphere", colormap="white-peru", zlim=c(0,6), newpage=F, vertex.size=12, vertex.label.color="blue", vertex.label.dist=0.5, vertex.label.font=3, glayout=layout_with_kk)

####################################
# identify common genes beween autoinflammatory and autoimmune
net_in_im <- graph.intersection(subnet_in, subnet_im, keep.all.vertices=T)
allnodes <- V(net_in_im)$name
df_id <- cbind(match(allnodes, V(subnet_in)$name), match(allnodes, V(subnet_im)$name))
...
# identify common genes beween autoinflammatory and mixed
net_in_mx <- graph.intersection(subnet_in, subnet_mx, keep.all.vertices=T)
allnodes <- V(net_in_mx)$name
df_id <- cbind(match(allnodes, V(subnet_in)$name), match(allnodes, V(subnet_mx)$name))
pattern <- apply(!is.na(df_id), 1, sum)
names(pattern) <- allnodes
g <- net_in_mx
xVisNet(g, pattern=pattern, colormap="wyr", zlim=c(0,2), vertex.shape="sphere", vertex.label.font=2, newpage=F)

# identify common genes beween autoimmune and mixed
net_im_mx <- graph.intersection(subnet_im, subnet_mx, keep.all.vertices=T)
allnodes <- V(net_im_mx)$name
df_id <- cbind(match(allnodes, V(subnet_im)$name), match(allnodes, V(subnet_mx)$name))
pattern <- apply(!is.na(df_id), 1, sum)
names(pattern) <- allnodes
g <- net_im_mx
xVisNet(g, pattern=pattern, colormap="wyr", zlim=c(0,2), vertex.shape="sphere", vertex.label.font=2, newpage=F)
...
library(VennDiagram)
data <- list()
data$inf <- V(subnet_in)$name
data$imm <- V(subnet_im)$name
data$mix <- V(subnet_mx)$name
vp <- venn.diagram(x=data, filename=NULL, fill=c("skyblue","pink1","green"), category.names=names(data))
grid.draw(vp)

## gene info
ig <- xRDataLoader(RData.customised='org.Hs.string', RData.location=RData.location)
allGenes <- igraph::get.data.frame(ig, what="vertices")
iGenes <- allGenes[, 3:5]
colnames(iGenes) <- c("GeneID","Symbol","Description")

### merge
allnodes <- unique(unlist(data))
mat_merged <- matrix(0, nrow=length(allnodes), ncol=3)
colnames(mat_merged) <- c('Autoimmune', 'Mixed', 'Autoinflammatory')
rownames(mat_merged) <- allnodes
mat_merged[match(data$imm,allnodes), 1] <- 1
mat_merged[match(data$mix,allnodes), 2] <- 1
mat_merged[match(data$inf,allnodes), 3] <- 1
code <- apply(mat_merged, 1, function(x) paste(x,collapse=''))
count <- apply(mat_merged, 1, sum)
df_merged <- data.frame(mat_merged, count=count, stringsAsFactors=F)
### order
ind <- with(df_merged, order(-count, -Autoimmune, -Mixed, -Autoinflammatory, rownames(df_merged)))
df_merged_order <- df_merged[ind, ]
### add gene info
ind <- match(rownames(df_merged_order), iGenes$Symbol)
df_info <- iGenes[ind,]
df_all <- cbind(df_info, df_merged_order)
rownames(df_all) <- rownames(df_merged_order)
write.table(df_all, file="network.genes.txt", sep="\t", row.names=F)

## ggplot
data <- df_all[,c(6,5,4)]
visHeatmapAdv(data, Rowv=F, Colv=F, dendrogram="none", key=F, zlim=c(0,1), colormap="grey-steelblue", add.expr=abline(v=(1:(ncol(data)+1))-0.5,h=(1:(nrow(data)+1))-0.5,,col="white"), lmat=rbind(c(4,3), c(2,1)), lhei=c(1,9), lwid=c(1,9), margins=c(4,8), cexCol=1, srtRow=-30, offsetRow=0.1, adjRow=0.2)
