---
title: "XGR: eXploring Genomic Relations at the gene and SNP level through enrichment analysis, similarity analysis and network analysis"
author: | 
...
We introduce an R package called *XGR* [^1]. This package is designed to make a user-defined gene list (or SNP list) more interpretable and discoverable: able to reveal the underlying knowledge by comprehensively utilising ontology annotations and network information. XGR is unique in supporting a broad range of ontologies (covering knowledge on functions, pathways, diseases, and phenotypes- in both human and mouse) and different types of networks (including functional, physical and pathway interactions). After going through this user manual (particularly the section `Applications` for the demo with real cases), you will be able to: 1) perform enrichment analysis using either built-in or custom ontologies, 2) calculate semantic similarity between genes (or between SNPs) based on their ontology annotation profiles, and 3) identify a gene subnetwork given your query list of (significant) genes or SNPs. For end-users who are unfamiliar with R, please refer to our user-friendly web app available [here](http://galahad.well.ox.ac.uk/XGR).
...
Auxiliary functions provide supplementary support during the package development, such as code debugging and documentation creating. 
...
# by default, RData.location <- "https://github.com/hfang-bristol/RDataCentre/blob/master/XGR/1.0.0"
```
...
```{r, eval=T, cache=T}
data <- cis_df$eQTL
ig_SNP <- xSocialiserSNPs(data=data, ontology="EF", RData.location=RData.location)
...
```{r, eval=T, cache=T}
data <- cis_df$eQTL
ig_SNP_LD <- xSocialiserSNPs(data=data, ontology="EF", include.LD="EUR", LD.r2=0.8, RData.location=RData.location)
...
## Analysing SNPs identified from GWAS

The second dataset we use is GWAS lead SNPs associated with immunologically related human diseases, obtained from [`ImmunoBase`](http://www.immunobase.org). 
...
# create a data frame
df <- data.frame(code=names(ImmunoBase), disease=unlist(disease_list), num_SNPs=unlist(variants_list), num_nearby_genes=unlist(genes_list), stringsAsFactors=F)
```

A summary of diseases and their GWAS lead SNPs plus nearby genes:

```{r, eval=T, echo=F}
knitr::kable(df, caption="", row.names=F)
```

In the following subsections, we focus on two inflammatory diseases, Ankylosing Spondylitis (AS) and Systemic Lupus Erythematosus (SLE), to compare putative gene networks that are likely modulated by their corresponding lead (or LD) SNPs. The applications in the rest diseases can be done similarly.

### SNP-modulated gene network in Ankylosing Spondylitis (AS)

```{r, eval=T, cache=T}
# get SNPs reported in AS GWAS and their significance info (p-values)
gr <- ImmunoBase$AS$variant
data <- as.matrix(mcols(gr)[, c('Variant','Pvalue')])

# find maximum-scoring gene subnetwork with the desired node number=30
subnet_AS <- xSubneterSNPs(data=data, network="STRING_high", distance.max=500000, seed.genes=T, subnet.size=30, RData.location=RData.location)
```

The identified gene network with nodes colored according to -log10(p-values) is shown below:

```{r, fig.width=8, fig.height=8, eval=T, cache=T, echo=T}
pattern <- -log10(as.numeric(V(subnet_AS)$significance))
pattern[is.infinite(pattern)] <- max(pattern[!is.infinite(pattern)])
vmax <- ceiling(stats::quantile(pattern, 0.75))
vmin <- floor(min(pattern))
xVisNet(g=subnet_AS, pattern=pattern, glayout=layout_(subnet_AS, with_kk()), vertex.shape="sphere", colormap="yr", zlim=c(vmin,vmax), newpage=F, edge.arrow.size=0.3, vertex.label.color="blue", vertex.label.dist=0.35, vertex.label.font=2)
```

### SNP-modulated gene network in Systemic Lupus Erythematosus (SLE)

```{r, eval=T, cache=T}
# get SNPs reported in SLE GWAS and their significance info (p-values)
gr <- ImmunoBase$SLE$variant
data <- as.matrix(mcols(gr)[, c('Variant','Pvalue')])

# find maximum-scoring gene subnetwork with the desired node number=30
subnet_SLE <- xSubneterSNPs(data=data, network="STRING_high", distance.max=500000, seed.genes=T, subnet.size=30, RData.location=RData.location)
```

The identified gene network with nodes colored according to -log10(p-values) is shown below:

```{r, fig.width=8, fig.height=8, eval=T, cache=T, echo=T}
pattern <- -log10(as.numeric(V(subnet_SLE)$significance))
pattern[is.infinite(pattern)] <- max(pattern[!is.infinite(pattern)])
vmax <- ceiling(stats::quantile(pattern, 0.75))
vmin <- floor(min(pattern))
xVisNet(g=subnet_SLE, pattern=pattern, glayout=layout_(subnet_SLE, with_kk()), vertex.shape="sphere", colormap="yr", zlim=c(vmin,vmax), newpage=F, edge.arrow.size=0.3, vertex.label.color="blue", vertex.label.dist=0.35, vertex.label.font=2)
```


# Session Info

Here is the output of `sessionInfo()` on the system on which this vignette was built:
