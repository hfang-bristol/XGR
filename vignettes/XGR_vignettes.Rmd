...
RData.location <- "/Users/hfang/Sites/SVN/github/RDataCentre/Portal"
...
res_name <- lapply(ImmunoBase, function(x){
	x$disease
})
res_fullname <- unlist(res_name)


res_seed <- lapply(ImmunoBase, function(x){
	gr <- x$variant
	data <- as.matrix(mcols(gr)[, c('Variant','Pvalue')])
	mSeed <- xSNP2GeneScores(data, include.LD="EUR", LD.r2=0.8, significance.threshold=5e-5, distance.max=200000, decay.kernel=c("slow","linear","rapid"), decay.exponent=2, GR.SNP="dbSNP_GWAS", GR.Gene="UCSC_genes", scoring.scheme=c("max","sum","sequential"), verbose=T, RData.location=RData.location)
	mSeed
})

res_SNP <- lapply(res_seed, function(x){
	x$SNP$SNP
})
allSNPs <- unique(unlist(res_SNP))

res_Gene <- lapply(res_seed, function(x){
	x$Gene$Gene
})
...
## 
data <- mat_SNP
visHeatmapAdv(data, Rowv=T, Colv=F, dendrogram="none", dist.metric=c("euclidean","pearson","spearman","kendall","manhattan","cos","mi")[1], colormap="wyr", labRow=NA, KeyValueName="SNP score", add.expr=abline(v=(1:(ncol(data)+1))-0.5,col="white"), lmat=rbind(c(4,3), c(2,1)), lhei=c(1,5), lwid=c(1,3), srtCol=30, margins=c(8,2))

visTreeBootstrap(data=t(mat_SNP), algorithm="nj", metric=c("euclidean","pearson","spearman","cos","manhattan","kendall","mi")[1], num.bootstrap=2000, consensus=F, consensus.majority=0.5, reroot="min.bootstrap", visTree=T, nodelabels.arg=list(cex=0.2))
      

## 
data <- mat_Gene
visHeatmapAdv(data, Rowv=T, Colv=F, dendrogram="none", dist.metric=c("euclidean","pearson","spearman","kendall","manhattan","cos","mi")[1], colormap="wyr", labRow=NA, KeyValueName="Gene score", add.expr=abline(v=(1:(ncol(data)+1))-0.5,col="white"), lmat=rbind(c(4,3), c(2,1)), lhei=c(1,5), lwid=c(1,3), srtCol=30, margins=c(8,2))

visTreeBootstrap(data=t(mat_Gene), algorithm="nj", metric=c("euclidean","pearson","spearman","cos","manhattan","kendall","mi")[1], num.bootstrap=2000, consensus.majority=0.5, reroot="min.bootstrap", visTree=T, nodelabels.arg=list(cex=0.8))


tree_SNPs <- visTreeBootstrap(data=t(mat_SNP), algorithm="nj", metric=c("euclidean","pearson","spearman","cos","manhattan","kendall","mi")[1], num.bootstrap=2000, consensus=T, consensus.majority=0.5, reroot="min.bootstrap", visTree=T, nodelabels.arg=list(cex=0.8))

tree_Genes <- visTreeBootstrap(data=t(mat_Gene), algorithm="nj", metric=c("euclidean","pearson","spearman","cos","manhattan","kendall","mi")[1], num.bootstrap=2000, consensus=T, consensus.majority=0.5, reroot="min.bootstrap", visTree=T, nodelabels.arg=list(cex=0.8))
