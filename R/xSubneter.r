...
#' subnet <- xSubneter(data=data, network="STRING_high", subnet.significance=0.01)
#' # b2) find maximum-scoring subnet with the desired node number=50
#' subnet <- xSubneter(data=data, network="STRING_high", subnet.size=50)
#'
...
xSubneter <- function(data, network=c("STRING_highest","STRING_high","STRING_medium","PCommonsUN_high","PCommonsUN_medium","PCommonsDN_high","PCommonsDN_medium","PCommonsDN_Reactome","PCommonsDN_KEGG","PCommonsDN_HumanCyc","PCommonsDN_PID","PCommonsDN_PANTHER","PCommonsDN_ReconX","PCommonsDN_TRANSFAC","PCommonsDN_PhosphoSite","PCommonsDN_CTD"), network.customised=NULL, subnet.significance=0.01, subnet.size=NULL, verbose=T, RData.location="https://github.com/hfang-bristol/RDataCentre/blob/master/XGR/0.99.0")
{
    ...
    if(!is.null(network.customised) && class(ig)=="igraph"){
		if(verbose){
			now <- Sys.time()
